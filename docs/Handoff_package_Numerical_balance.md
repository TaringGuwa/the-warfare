# HAND-OFF PACKAGE — Numerical Balance Specialist
**Proyek:** Sistem Perancangan GDD & Numerical Balancing — Pilot "The Warfare"
**Dari:** PM (Advisor session, Claude)
**Tanggal:** 19 September 2026
**Mekanisme:** Relay Document manual, sesuai Masterplan System Maker Section 15

---

## Peran Anda

Anda adalah **Numerical Balance Specialist** dalam tim AI-Agent proyek ini. Tugas Anda **khusus menangani angka/balancing**, bukan world-building atau prose GDD.

## Konteks Ringkas

Proyek ini membangun "The Warfare" — game RTS — dan sedang melalui proses audit kelengkapan GDD + Numerical Balancing Draft. Anda menerima **satu temuan spesifik** untuk dikerjakan.

## Tugas: Temuan #3

**Masalah:** GDD Section 4.3 menyatakan secara langsung:
> "Tank: ... in Mountain/Forest: speed drops one additional tier (extremely slow) and **power is reduced by 80%**."

Angka **80%** ini:
1. Ditulis seolah sudah pasti/final di GDD, **tidak** ditandai `DRAFT`.
2. **Tidak muncul** di `Numeric_Balancing_Draft.md` — padahal GDD Section 22 mengklaim "no numeric constant remains fully unassigned" (menyiratkan semua 14 item numerik sudah lengkap di companion doc; angka ini adalah item ke-15 yang terlewat).
3. Numeric Balancing Draft Section 8.3 HANYA mencatat penalti **waktu** Tank di Mountain/Forest (+2 hari), tidak menyebut penalti **power** sama sekali.

**Yang perlu Anda kerjakan:**
1. Tentukan apakah 80% ini harus diperlakukan sebagai `DRAFT` (ikut proses balancing terstruktur seperti 14 item lain) atau memang layak `LOCKED` — beri alasan.
2. Cek interaksi angka ini dengan formula lain yang sudah ada (terutama Attack Success Chance, Section 4.1 Balancing Draft, dan Frontline Pressure, Section 4.2) — apakah 80% ini masuk sebagai modifier terpisah, atau harus dikombinasikan ke Power Ratio sebelum masuk formula Success%?
3. **Output yang diminta:** draft subsection baru untuk Numerical Balancing Draft (format: ikuti konvensi dokumen yang sudah ada — tabel, tag status, rasional, entry Changelog Section 12) yang secara resmi meregister angka ini.
4. **Jangan** mengubah angka 80%-nya sendiri kecuali Anda punya alasan kuat (ini bukan tugas re-balancing, ini tugas *migrasi ke format tracking yang benar*).

## Batasan (Human Control — Section 14 Masterplan)

Ini kerjaan **Design/Analysis tingkat Medium risk**. Anda boleh membuat draft & rekomendasi, tapi **JANGAN** menyatakan sesuatu "final/LOCKED" — human yang approve. Beri rekomendasi eksplisit (format: **"Rekomendasi AI:"**) tapi jangan putuskan sendiri.

## Format Laporan Balik

Tulis output Anda diberi label role & tanggal, siap ditempel balik ke `context-log.md` proyek ini:

```
### [Numerical Balance Specialist — <tanggal>]
<isi laporan>
```

---

## LAMPIRAN 1 — context-log.md (utuh)

# CONTEXT LOG — Sistem Perancangan GDD & Numerical Balancing
**Pilot proyek:** The Warfare
**Dibuat:** 19 September 2026
**Mengikuti:** System Maker Masterplan v1.2, Section 15 (Relay Document mechanism)

---

## Project-Foundation (ringkasan)

- **Scope sistem target:** Metodologi umum untuk merancang & melengkapi GDD serta numerical balancing game — reusable lintas proyek, bukan cuma The Warfare.
- **Batas:** Berhenti di layer desain & angka. Tidak mencakup coding/implementasi, art, marketing.
- **Complexity Tier:** Complex (multi-role, direncanakan tim AI multi-role formal).
- **Mode komunikasi human:** ELI5 aktif.

## Team Formation Checkpoint — LOCKED (19 Sep 2026)

Opsi C dipilih & disetujui human — 5 role:

| Role | Tanggung jawab |
|---|---|
| PM | Jaga context-log, koordinasi, cegah gate terlewat |
| World/Content Designer | Isi gap dunia game (map, fitur, aturan) |
| Numerical Balance Specialist | Angka & balancing |
| Consistency & QA Critic | Cross-check kontradiksi antar-dokumen |
| Verificator | Cek hasil akhir vs spec asli sebelum "selesai" |

**Catatan ground truth (19 Sep 2026):** Dikonfirmasi human — tidak ada dokumen map terpisah. `the_warfare_concept_v0.0.8.md` adalah induk konten, isinya sudah terserap penuh ke GDD v0.0.2 (GDD = versi full). Tidak perlu dokumen tambahan itu untuk analisis lanjutan.

---

## Analysis Findings (Analyst/Critic, 19 Sep 2026)

Sumber: `GDD_The_Warfare_v0.0.2.md` + `The_Warfare_Numeric_Balancing_Draft_v0.2.md` (dibaca lengkap).

| # | Temuan | Kategori | Status |
|---|---|---|---|
| 1 | Data map/wilayah tidak ada di dokumen manapun | Dunia game | **RESOLVED (LOCKED)** — disetujui human 19 Sep 2026. Lihat `GDD_Addendum_Section19_MapLayout.md` |
| 2 | "AD" tidak didefinisikan di Glossary | Istilah/definisi | **RESOLVED** — AD = Angkatan Darat (dikonfirmasi human 19 Sep 2026). Lihat `GDD_Patch_Temuan_2_5_6.md` Patch 1 |
| 3 | Tank power penalty 80% (Section 4.3) tidak masuk Numerical Balancing register, tidak ditandai DRAFT | Konsistensi numerik | **IN PROGRESS — INI TUGAS ANDA** |
| 4 | Defensive Adjacency: GDD 6.4 (tambah effective defense) vs Balancing 4.1 (kurangi Success% attacker langsung) — dua mekanisme beda | Konsistensi mekanik | **RESOLVED (LOCKED)** — disetujui human: Opsi 2 (modifier langsung ke Success%). Lihat `GDD_Patch_Temuan_2_5_6.md` Patch 4 |
| 5 | Military building "max 2 per tipe per map" berpotensi terlalu restriktif utk 15 region | Potensi desain janggal | **RESOLVED** — dikonfirmasi human: disengaja (bottleneck strategis). Lihat `GDD_Patch_Temuan_2_5_6.md` Patch 2 |
| 6 | "Policy development" disebut pakai Money (3.1) tapi tidak ada policy berbayar yang dijelaskan | Konsistensi/dunia game | **RESOLVED** — dikonfirmasi human: leftover teks lama, dihapus. Lihat `GDD_Patch_Temuan_2_5_6.md` Patch 3 |

**Risiko carry-over dari Balancing doc (relevan untuk tugas Anda):**
- RISK-001: interaksi Regional Base Output (1.3) ↔ Economic AI threshold (9.3), pending AI-Behavior Specialist review.
- RISK-002: Development Systems (4.1/4.2) belum punya cost basis sendiri.

---

## Decision Log

- 19 Sep 2026 — Sistem target ditetapkan: metodologi umum (general), The Warfare = pilot pertama.
- 19 Sep 2026 — Team Formation: Opsi C (5 agent), disetujui human.
- 19 Sep 2026 — Prioritas kerja: Temuan #1 dikerjakan lebih dulu.
- 19 Sep 2026 — Struktur grid map direkonstruksi (bukan disalin dari sumber lama yang ternyata tidak eksis) — logika divalidasi cocok dengan P6/E10 existing.
- 19 Sep 2026 — Temuan #1 dan #4 disetujui human (LOCKED). Temuan #2, #5, #6 dikonfirmasi human & selesai.

---

*(Catatan untuk Anda, Numerical Balance Specialist: setelah selesai, tempel laporan Anda kembali ke bagian bawah context-log.md ini dengan format label role & tanggal seperti dicontohkan di badan hand-off ini.)*

---

## LAMPIRAN 2 — Numeric_Balancing_Draft.md (utuh, WAJIB dilampirkan penuh sesuai aturan ground truth Section 15)
...
**CATATAN PENTING untuk Numerical Balance Specialist:** Perhatikan Section 4.1 di atas — modifier "Defensive Adjacency" di situ **sudah benar/tidak berubah** (ini yang jadi acuan resmi setelah Temuan #4 di-lock). Yang direvisi hanya narasi di GDD Section 6.4, bukan formula di sini.
